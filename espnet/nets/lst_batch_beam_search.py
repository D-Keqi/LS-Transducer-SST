"""LS-Transducer-SST Parallel beam search module."""
...
from espnet.nets.LST_beam_search import LSTBeamSearch
...
class LSTBatchBeamSearch(LSTBeamSearch):
    ...
    def search(
        self, running_hyps: BatchHypothesis, x: torch.Tensor, search_1=False
    ) -> BatchHypothesis:
        ...
        if search_1:
            ...
            if (
                self.part_scorers["stctc"].impl.enable_eos
                and self.part_scorers["stctc"].is_final
            ):
                self.beam_size = 10
            elif not self.part_scorers["stctc"].chunk_prune:
                self.beam_size = 10
            else:
                self.beam_size = 1
