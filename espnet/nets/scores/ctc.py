...
class CTCPrefixScorer_online(BatchPartialScorerInterface):
    """Decoder interface wrapper for CTCPrefixScore."""
    ...
    def init_state(self, x: torch.Tensor):
        ...
        """
        logp = self.ctc.log_softmax(x.unsqueeze(0)).detach().squeeze(0).cpu().numpy()
        logp = logp[:, : self.chunk_size]  # default chunk size is 64
        # TODO(karita): use CTCPrefixScoreTH
        self.impl = CTCPrefixScoreTH(
            logp, torch.LongTensor([len(logp)]).to(logp.device), 0, self.eos
        )
        self.impl.tmp = logp
        return 0, self.impl.initial_state()
    ...
    def batch_init_state(self, x: torch.Tensor):
        ...
        """
        logp = self.ctc.log_softmax(x.unsqueeze(0))  # assuming batch_size = 1
        logp = logp[:, : self.chunk_size]
        xlen = torch.tensor([logp.size(1)])
        self.impl = CTCPrefixScoreTH(logp, xlen, 0, self.eos)
        self.impl.tmp = logp[0]
        return None
    ...
    def batch_score_partial(self, y, ids, state, x):
        ...
        """
        weight = x[:, -1]
        alphas = 0.95 * torch.sigmoid(weight) + 0.05
        now_length = y.size(1)
        accum = 0
        cur_step = 0
        next_step = 0
        beta = 0  # adjust the threshold of AIF
        for n, i in enumerate(alphas):
            accum += i
            if accum >= (now_length + beta) or n == alphas.size(0) - 1:
                cur_step = n
                break
        accum = 0
        for n, i in enumerate(alphas):
            accum += i
            if accum >= (now_length + 1 + beta) or n == alphas.size(0) - 1:
                next_step = n
                break

        num_chunk = int(cur_step / self.chunk_size) + 1
        next_num_chunk = int(next_step / self.chunk_size) + 1
        if num_chunk * self.chunk_size >= alphas.size(0) and self.is_final:
            self.impl.enable_eos = True
        else:
            self.impl.enable_eos = False

        if next_num_chunk > num_chunk:
            self.chunk_prune = True
        else:
            self.chunk_prune = False

        if num_chunk * self.chunk_size > self.impl.tmp.size(0):

            logp = x[: int(num_chunk * self.chunk_size)]
            self.extend_prob(logp)
            state = self.extend_state(state)
            self.impl.tmp = x[: int(num_chunk * self.chunk_size)]
