...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def forward(
        ...
        # Streaming wav2vec2.0
        original_padding_mask = padding_mask.clone()
        chunk_size = 64  # 64*20/2=640 ms
        seq_len = padding_mask.shape[-1]
        s = np.arange(0, seq_len, chunk_size)
        enc_mask = (
            adaptive_enc_mask(seq_len, s)
            .unsqueeze(0)
            .expand([padding_mask.shape[0], -1, -1])
            .to(padding_mask.device)
        )
        padding_mask = ~padding_mask.unsqueeze(-2)
        enc_mask = enc_mask[:, : padding_mask.shape[2], : padding_mask.shape[2]]
        padding_mask = enc_mask & padding_mask
        ...
        if features_only:
            return {
                "x": x,
                "padding_mask": original_padding_mask,
...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.pos_conv = nn.Sequential(
            self.pos_conv, SamePad(args.conv_pos, True), nn.GELU()
        )
    ...
    def forward(self, x, padding_mask=None, original_padding_mask=None, layer=None):
        x, layer_results = self.extract_features(
            x, padding_mask, original_padding_mask, layer
        )
    ...
    def extract_features(
        self, x, padding_mask=None, original_padding_mask=None, tgt_layer=None
    ):

        if padding_mask is not None:
            x = index_put(x, original_padding_mask, 0)
