...
class CAATSpeech2Text:
    ...
    def __init__(
        ...
        st_model.joint_network.downsample = -1
        if enh_s2t_task:
        ...
        if st_model.use_transducer_decoder:
            beam_search_transducer = BeamSearchTransducer(
                decoder=st_model.decoder,
                joint_network=st_model.joint_network,
                beam_size=beam_size,
    ...
    @torch.no_grad()
    def __call__(self, speech: Union[torch.Tensor, np.ndarray]) -> List[
        Tuple[
            Optional[str],
            List[str],
            List[int],
    ...
    def _decode_single_sample(self, enc: torch.Tensor):
        ...
        for hyp in nbest_hyps:
            ...
            # remove sos/eos and get results
            last_pos = None if self.st_model.use_transducer_decoder else -1
