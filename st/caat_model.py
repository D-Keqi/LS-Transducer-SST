...
class CAATModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        assert 0.0 <= stmtlalpha <= 1.0, "mtlalpha should be [0.0, 1.0]"
        ...
        if self.use_transducer_decoder:
            ...
            self.criterion_transducer = DelayTLoss(
                blank=self.blank_id,
                delay_scale=1.0,
                temperature=1.0,
                reduction="sum",
                delay_func="diag_positive",
            )
        ...
        # CTC/attention joint translation
        if self.stmtlalpha > 0:
            self.stctc = ctc
            self.stctc_error_calculator = ASRErrorCalculator(
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 2a.a CTC/attention joint stctc branch
        if self.stmtlalpha > 0:
            loss_st_ctc, cer_st_ctc = self._calc_stctc_loss(
                encoder_out, encoder_out_lens, text, text_lengths
            )
        else:
            loss_st_ctc, cer_st_ctc = 0, None
        ...
        # 3. Loss computation
        if self.use_transducer_decoder:
            loss_st_my = loss_transducer
        else:
            loss_st_my = loss_st_att
        asr_ctc_weight = self.mtlalpha
        st_ctc_weight = self.stmtlalpha
        loss_st = (1 - st_ctc_weight) * loss_st_my + st_ctc_weight * loss_st_ctc
        ...
        stats = dict(
            loss=loss.detach(),
            loss_asr=(
                loss_asr.detach() if type(loss_asr) not in {float, int} else loss_asr
            ),
            ...
            wer=wer_asr_att,
            bleu=bleu_st_att,
            loss_transducer=loss_transducer,
    ...
    def _calc_stctc_loss(
        ...
        # Calc CTC loss
        loss_ctc = self.stctc(encoder_out, encoder_out_lens, ys_pad, ys_pad_lens)

        # Calc CER using CTC
        cer_ctc = None
        if not self.training and self.stctc_error_calculator is not None:
            ys_hat = self.stctc.argmax(encoder_out).data
            cer_ctc = self.stctc_error_calculator(
    ...
    def _calc_transducer_loss(
        ...
        memory_mask = (make_pad_mask(encoder_out_lens, maxlen=encoder_out.size(1))).to(
            encoder_out.device
        )

        joint_out, group_lengths = self.joint_network(
            encoder_out.transpose(0, 1), memory_mask, decoder_out
        )

        x = joint_out
        src_lengths = group_lengths
        B, G, T, D = x.shape

        idx_pos = (src_lengths - 1).view(-1, 1, 1, 1).expand(B, 1, T, D)
        last_h = torch.gather(x, 1, idx_pos).squeeze(1).contiguous()
        last_h = last_h[:, :-1].contiguous()

        loss_ce = self.criterion_st(last_h, target.long())

        loss_transducer, _, _ = self.criterion_transducer(
            joint_out,
            target,
            group_lengths.int(),
            u_len,
        ...
        loss_transducer = loss_transducer / B + loss_ce
