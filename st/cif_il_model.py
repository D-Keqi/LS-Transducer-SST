...
class CIFSTModel(AbsESPnetModel):
    """CIF infinite lookback (CIF-IL) model"""
    ...
    def _calc_mt_att_loss(
        ...
        # 1. Forward decoder
        if st:
            decoder_out, _, diff, latency_loss = self.decoder(
                encoder_out, encoder_out_lens, ys_in_pad, ys_in_lens, ys_out_pad
            )
        else:
